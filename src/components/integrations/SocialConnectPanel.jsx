        ...
        <Card className="glass-card p-4">
          ...
          {tmEvents.length > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {tmEvents.map((ev, i) => {
                const img = ev.images?.find(img => img.ratio === "16_9") || ev.images?.[0];
                ...
                return (
                  <motion.div key={ev.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
                    <div className="rounded-xl overflow-hidden border border-[#2A2D3A] bg-[#0F1117] hover:border-[#00BFFF]/30 transition-colors">
                      {img && <img src={img.url} alt={ev.name} className="w-full h-24 object-cover" />}
                      <div className="p-3">
                        ...
                        <div className="flex items-center gap-1 mt-1 text-[10px] text-[#8B8D97]">
                          {date && <><Calendar className="w-2.5 h-2.5" /> {date}</>}
                          {venue && <><MapPin className="w-2.5 h-2.5 ml-2" /> {venue.name}</>}
                        </div>
                        {minPrice && <p className="text-[10px] text-emerald-400 mt-1">From ${minPrice}</p>}
