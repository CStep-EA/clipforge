      ...
      <div className={cn(
        ...
        <div
          ...
          onKeyDown={(e) => {
            const vid = videoRef.current;
            if (!vid || !isFinite(vid.duration) || vid.duration === 0) return;
            const next = e.key === "ArrowRight" ? Math.min(vid.duration, vid.currentTime + 5)
                       : e.key === "ArrowLeft"  ? Math.max(0, vid.currentTime - 5)
                       : null;
            if (next !== null && isFinite(next)) vid.currentTime = next;
          }}
        >
          <div
